...
exports.getChats = async (req, res) => {
  ...
  dd="CASE    WHEN MATCH(chats.message) AGAINST('"+req.query.search+"' IN NATURAL LANGUAGE MODE) > 0 THEN 1    ELSE 0  END AS match_status,"
...
if (req.query.is_group != 1 || req.query.is_group != 2) {
    sql1 =
      "SELECT chats.*, FROM `chats` LEFT JOIN users ON users.id=chats.send_by WHERE chats.send_by IN(" +
      req.query.login_user_id +
      "," +
      req.query.user_id +
      ") AND chats.sent_to IN(" +
...
sql =
      "SELECT chats.*,"+dd+"  case when chats.images IS NOT NULL then chats.images   else ''  end AS images, CONCAT('" +
      constants.BASE_URL +
      "','images/profiles/',users.profile_picture) AS profile_picture,users.name FROM `chats` LEFT JOIN users ON users.id=chats.send_by WHERE chats.send_by IN(" +
      req.query.login_user_id +
      "," +
...
exports.getDirectMessages = async (req, res) => {
  ...
  sql =
    ...
    req.query.login_user_id +
    " AND is_seen=0 ) ELSE 0 END AS newMessageCount FROM users LEFT JOIN users_requests ON (   users.id =  case when users_requests.user_id!=" +
    req.query.login_user_id +
