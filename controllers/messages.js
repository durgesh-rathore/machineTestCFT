...
exports.getSpiltChats = async (req, res) => {
  ...
  if (
    ...
    sql1 =
      ...
      req.query.login_user_id +
      "  ) AS group_users_image,(select sum( case when billing_group_users.payment_amount IS NOT NULL then billing_group_users.payment_amount else 0 end )/COUNT(*)  from `billing_group_users` WHERE billing_group_users.group_id=billing_group.group_id ) AS percentage  FROM billing_group  LEFT JOIN billing_group_users ON billing_group_users.group_id=billing_group.id  LEFT JOIN users ON users.id=billing_group_users.group_id     WHERE billing_group_users.user_id=" +
      req.query.login_user_id;
