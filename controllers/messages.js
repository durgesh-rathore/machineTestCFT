...
exports.getDirectMessages = async (req, res) => {
  ...
  connection.query(sql, function (err, directMessages) {
    ...
    sqlCountsDM= "SELECT  COUNT(*) AS counts  FROM users LEFT JOIN users_requests ON (   users.id =  case when users_requests.user_id<>" +
    req.query.login_user_id +
    " Then users_requests.user_id ELSE users_requests.request_for END)  LEFT JOIN groups_users ON groups_users.group_id= users.id WHERE  ( (users_requests.user_id='"+req.query.login_user_id +"' OR users_requests.request_for='"+req.query.login_user_id +"') AND ( users_requests.is_reject=0 AND users_requests.is_block=0 AND users_requests.is_accepted=1 )  OR (users.is_group=1 AND groups_users.user_id='"+req.query.login_user_id +"')  )  AND users.id <> '"+req.query.login_user_id +"' "  + search + "  GROUP BY users.id "
    ...
    //   req.query.login_user_id;
      ...
      var sqlCountsSplit= "SELECT COUNT(*) AS counts    FROM users AS users2 LEFT JOIN billing_group ON billing_group.group_id=users2.id  LEFT JOIN billing_group_users ON billing_group_users.group_id=billing_group.group_id  LEFT JOIN users AS user1 ON user1.id=billing_group_users.user_id  WHERE users2.is_group=2 AND billing_group_users.user_id=" +
      req.query.login_user_id +
...
exports.getSpiltChats = async (req, res) => {
  ...
  if (
    ...
    // " 
    sql1 =
      "SELECT users.name,users.group_admin_id,(  SELECT  CASE WHEN billing_group_users.payment_amount IS NOT NULL THEN 1 ELSE 0 END FROM  billing_group_users WHERE billing_group_users.group_id= "+req.query.group_id +" AND billing_group_users.user_id=" +
      req.query.login_user_id +
      "  ) AS is_paid, billing_group.group_id,billing_group.spliting_amount,( SELECT GROUP_CONCAT(users.profile_picture) FROM users LEFT JOIN billing_group_users ON billing_group_users.user_id=users.id WHERE billing_group_users.group_id=billing_group.group_id  AND users.id <> " +req.query.login_user_id +") AS group_users_image,COUNT(*) AS contributors,(select  COUNT(*) from billing_group_users  WHERE billing_group_users.group_id=billing_group.group_id  AND  billing_group_users.payment_amount IS NOT NULL) AS paid_contributor,      (select  COUNT(*) from billing_group_users  WHERE billing_group_users.group_id=billing_group.group_id  AND  billing_group_users.payment_amount IS  NULL) AS pending_contributor,(select ROUND((sum( case when billing_group_users.payment_amount IS NOT NULL then billing_group_users.payment_amount else 0 end )/billing_group.spliting_amount) *100 ,2 ) from billing_group_users  WHERE billing_group_users.group_id=billing_group.group_id ) AS percentage,(select  CEIL(billing_group.spliting_amount/COUNT(*)) from billing_group_users WHERE billing_group_users.group_id=billing_group.group_id ) AS each_split ,billing_group.currency  FROM billing_group  LEFT JOIN billing_group_users ON billing_group_users.group_id=billing_group.group_id  LEFT JOIN users ON users.id=billing_group_users.group_id     WHERE billing_group_users.group_id=" +
