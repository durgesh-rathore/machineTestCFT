...
exports.getSpiltChats = async (req, res) => {
  ...
  if (
    ...
    sql1 =
      ...
      req.query.login_user_id +
      "  ) AS group_users_image,COUNT(*) AS contributors,(select  COUNT(*) from billing_group_users  WHERE billing_group_users.group_id=billing_group.group_id  AND  billing_group_users.payment_amount IS NOT NULL) AS paid_contributor,      (select  COUNT(*) from billing_group_users  WHERE billing_group_users.group_id=billing_group.group_id  AND  billing_group_users.payment_amount IS  NULL) AS pending_contributor,(select ROUND(((billing_group.spliting_amount-sum( case when billing_group_users.payment_amount IS NOT NULL then billing_group_users.payment_amount else 0 end ))/billing_group.spliting_amount) *100 ,2 ) from billing_group_users  WHERE billing_group_users.group_id=billing_group.group_id ) AS percentage,(select  CEIL(billing_group.spliting_amount/COUNT(*)) from `billing_group_users` WHERE billing_group_users.group_id=billing_group.group_id ) AS each_split   FROM billing_group  LEFT JOIN billing_group_users ON billing_group_users.group_id=billing_group.group_id  LEFT JOIN users ON users.id=billing_group_users.group_id     WHERE billing_group_users.group_id=" +
